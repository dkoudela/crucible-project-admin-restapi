...
/**
 * Description: Plugin's REST API for Crucible Project's Creation operations.
...
public class ProjectNewRestApi
{
	...
	public ProjectNewRestApi(ProjectAdminModelImpl projectAdminModelImpl)
	{
		this.projectAdminModelImpl = projectAdminModelImpl;
	}

	/**
	 * Provides HTTP GET method for project Creation operation
	 *
	...
	@GET
	@Produces({MediaType.APPLICATION_JSON})
	public Response newProjectGet(@QueryParam("name") String name,
	...
	{
		return newProjectFacade(name, key, defaultRepositoryName, storeRevisions, permissionSchemeName,
				moderatorEnabled, defaultModerator, defaultDuration, defaultObjectives);
	}

	/**
	 * Provides HTTP POST method for project Creation operation
	 *
	 * @param name the plain language name as displayed in the Crucible interface
	 * @param key the project key used when giving reviews their unique code names
	 * @param defaultRepositoryName default FishEye repository name
	 ...
	 * @return ws response containing result of the operation
	 */
	private Response newProjectFacade(String name, String key, String defaultRepositoryName, String storeRevisions,
										 ...
										 String defaultDuration, String defaultObjectives)
	{
		try
		{
			return Response.ok().entity(projectAdminModelImpl.newProject(name, key, defaultRepositoryName, storeRevisions, permissionSchemeName,
																	 moderatorEnabled, defaultModerator, defaultDuration, defaultObjectives)).build();
		}
		catch (Exception e)
		{
			System.out.println("Exception in the ProjectNewRestApi: "+e.getMessage());
